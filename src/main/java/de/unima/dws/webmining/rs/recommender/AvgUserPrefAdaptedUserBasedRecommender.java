...
package de.unima.dws.webmining.rs.recommender;
 ...
 * </p>
 */
public class AvgUserPrefAdaptedUserBasedRecommender extends AbstractRecommender
		implements UserBasedRecommender {

	private static final Logger log = LoggerFactory
			.getLogger(AvgUserPrefAdaptedUserBasedRecommender.class);
	...
	public AvgUserPrefAdaptedUserBasedRecommender(DataModel dataModel,
	...
	/**
	 * This estimation is more optimal as it takes into account the avg user
	 * preferences
	 * 
	 * @param theUserID
	...
	protected float doEstimatePreference(long theUserID,
			...
			if (userID != theUserID) {
				...
				if (pref != null) {
					...
					if (!Double.isNaN(theSimilarity)) {
						float userAvgRating = calculateAverageRating(userID);
						preference += theSimilarity * (pref - userAvgRating);
		...
		float theUserAvgPref = calculateAverageRating(theUserID);
		estimate += theUserAvgPref;
		if (capper != null) {
	...
	/**
	 * This estimator evokes a more precise but a little bit slower estimation
	 * of preferences taking avg. user ratings into account
	 * 
	 * 
	 */
